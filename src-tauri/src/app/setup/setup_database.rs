...
use crate::app::entities::{
    order_book::Entity as OrdersEntity, subscriptions::Entity as SubscriptionsEntity,
...
pub async fn initalize_database() -> Result<DatabaseConnection, Box<dyn std::error::Error>> {
    ...
    let create_subscriptions_table =
        schema.create_table_from_entity(SubscriptionsEntity).if_not_exists().to_owned();
    db.execute(backend.build(&create_subscriptions_table)).await?;
    info!("Subscriptions table created.");

    // Create the trades table
    let create_trades_table =
        schema.create_table_from_entity(TradesEntity).if_not_exists().to_owned();
    db.execute(backend.build(&create_trades_table)).await?;
    info!("Trades table created.");

    // Create the orders table
    let create_orders_table =
        schema.create_table_from_entity(OrdersEntity).if_not_exists().to_owned();

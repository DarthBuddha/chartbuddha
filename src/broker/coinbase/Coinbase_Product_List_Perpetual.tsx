...
const Coinbase_Product_List_Perpetual: React.FC = () => {
  ...
  const [perpetualProducts, setPerpetualProducts] = useState<Type_BrokerProduct[]>([]);
  ...
  const loadPerpetualProducts = useCallback(async () => {
    try {
      ...
      const allProducts = ((await store_coinbase_products.get('products')) as { PERPS?: Type_BrokerProduct[] }) || {};
